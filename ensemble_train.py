...
def main():
    ...
    num_models = 10
    models = [dropout.FaceRecognitionModel(num_classes=num_classes).to(device) for _ in range(num_models)]
    ...
    for model_idx, (model, optimizer) in enumerate(zip(models, optimizers)):
        ...
        for epoch in range(num_epochs):
            ...
            print(f"Model {model_idx + 1}, Epoch {epoch+1}/{num_epochs}, Loss: {running_loss/len(train_loader)}, Train Accuracy: {accuracy_train}, Test Accuracy: {accuracy_test}")
        ...
        for i, model in enumerate(models):
            save_dir = os.path.dirname(f'saved_models/dropout/')
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            torch.save(model.state_dict(), f'saved_models/dropout/face_recognition_model_{i}.pth')
