...
class FaceRecognitionModel(nn.Module):
    def __init__(self, args=None, num_classes=20):
        ...
        self.dropout = nn.Dropout(p=0.5)  # Adding dropout layer with dropout probability 0.5
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)  # Applying dropout to the fully connected layer
        x = self.fc2(x)
        return x
...
class HyperGAN(HyperGAN_Base):
    ...
    def save_models(self, args, metrics=None):
        ...
        torch.save(save_dict, os.path.join(save_dir, f"epoch_{args.epochs}_div_{args.diversity_lambda}_thre_{args.threshold}.pth"))
